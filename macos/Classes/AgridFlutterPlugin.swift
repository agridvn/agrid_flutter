...
public class AgridFlutterPlugin: NSObject, FlutterPlugin {
    private static var instance: AgridFlutterPlugin?

    public static func getInstance() -> AgridFlutterPlugin? {
        instance
    }

    public static func register(with registrar: FlutterPluginRegistrar) {
        #if os(iOS)
            let channel = FlutterMethodChannel(name: "agrid_flutter", binaryMessenger: registrar.messenger())
        #elseif os(macOS)
            let channel = FlutterMethodChannel(name: "agrid_flutter", binaryMessenger: registrar.messenger)
        #endif
        let instance = AgridFlutterPlugin()
        instance.channel = channel
        AgridFlutterPlugin.instance = instance
        initPlugin()
        registrar.addMethodCallDelegate(instance, channel: channel)
    }

    private let dispatchQueue = DispatchQueue(label: "com.agrid.AgridFlutterPlugin",
                                              target: .global(qos: .utility))

    private var channel: FlutterMethodChannel?

    public static func initPlugin() {
        let autoInit = Bundle.main.object(forInfoDictionaryKey: "com.agrid.agrid.AUTO_INIT") as? Bool ?? true
        if !autoInit {
            print("[Agrid] com.agrid.agrid.AUTO_INIT is disabled!")
            return
        }

        let apiKey = Bundle.main.object(forInfoDictionaryKey: "com.agrid.agrid.API_KEY") as? String ?? ""

        let host = Bundle.main.object(forInfoDictionaryKey: "com.agrid.agrid.AGRID_HOST") as? String ?? PostHogConfig.defaultHost
        let captureApplicationLifecycleEvents = Bundle.main.object(forInfoDictionaryKey: "com.agrid.agrid.CAPTURE_APPLICATION_LIFECYCLE_EVENTS") as? Bool ?? false
        let debug = Bundle.main.object(forInfoDictionaryKey: "com.agrid.agrid.DEBUG") as? Bool ?? false
    ...
    private static func setupAgrid(_ agridConfig: [String: Any]) {
        guard let instance = AgridFlutterPlugin.instance else {
            print("[Agrid] Plugin instance not found!")
            return
        }
        let apiKey = agridConfig["apiKey"] as? String ?? ""
        if apiKey.isEmpty {
            print("[Agrid] apiKey is missing!")
            return
        }

        let host = agridConfig["host"] as? String ?? PostHogConfig.defaultHost
        ...
        if let captureApplicationLifecycleEvents = agridConfig["captureApplicationLifecycleEvents"] as? Bool {
            config.captureApplicationLifecycleEvents = captureApplicationLifecycleEvents
        }
        if let debug = agridConfig["debug"] as? Bool {
            config.debug = debug
        }
        if let flushAt = agridConfig["flushAt"] as? Int {
            config.flushAt = flushAt
        }
        if let maxQueueSize = agridConfig["maxQueueSize"] as? Int {
            config.maxQueueSize = maxQueueSize
        }
        if let maxBatchSize = agridConfig["maxBatchSize"] as? Int {
            config.maxBatchSize = maxBatchSize
        }
        if let flushInterval = agridConfig["flushInterval"] as? Int {
            config.flushIntervalSeconds = Double(flushInterval)
        }
        if let sendFeatureFlagEvents = agridConfig["sendFeatureFlagEvents"] as? Bool {
            config.sendFeatureFlagEvent = sendFeatureFlagEvents
        }
        if let preloadFeatureFlags = agridConfig["preloadFeatureFlags"] as? Bool {
            config.preloadFeatureFlags = preloadFeatureFlags
        }
        if let optOut = agridConfig["optOut"] as? Bool {
            config.optOut = optOut
        }

        if let personProfiles = agridConfig["personProfiles"] as? String {
        ...
        if let dataMode = agridConfig["dataMode"] as? String {
            switch dataMode {
            case "wifi":
        ...
        // Surveys and session replay not configured in this fork for macOS

        // Update SDK name and version
        postHogSdkName = "agrid-flutter"
        postHogVersion = agridFlutterVersion

        PostHogSDK.shared.setup(config)
    }

    // Surveys disabled on macOS in this fork
...
// Surveys disabled in this fork; no delegate conformance

extension AgridFlutterPlugin {
    ...
    private func setup(
        ...
        if let args = call.arguments as? [String: Any] {
            AgridFlutterPlugin.setupAgrid(args)
            result(nil)
        } else {
            _badArgumentError(result)
        }
    ...
    private func _badArgumentError(_ result: @escaping FlutterResult) {
        result(FlutterError(
            code: "AgridFlutterException", message: "Missing arguments!", details: nil
        ))
    }
