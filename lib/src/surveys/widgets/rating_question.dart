...
import '../models/agrid_display_survey_rating_type.dart';
...
class RatingQuestion extends StatefulWidget {
  const RatingQuestion({
    ...
    this.type = AgridDisplaySurveyRatingType.number,
  ...
  final AgridDisplaySurveyRatingType type;
...
class _RatingQuestionState extends State<RatingQuestion> {
  ...
  Widget _buildRatingButton(int value) {
    ...
    final range = widget.scaleUpperBound - widget.scaleLowerBound + 1;
    if (widget.type == AgridDisplaySurveyRatingType.emoji &&
        (range == 3 || range == 5)) {
      final buttonColor =
