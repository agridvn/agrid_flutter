...
import 'agrid_display_open_question.dart';
import 'agrid_display_link_question.dart';
import 'agrid_display_rating_question.dart';
import 'agrid_display_choice_question.dart';
import 'agrid_display_survey_appearance.dart';
import 'agrid_display_survey_text_content_type.dart';

/// Main survey model containing metadata and questions
@immutable
class AgridDisplaySurvey {
  // Convert back from survey dictionary to a Dart object
  // Native platform model -> Dictionary -> Dart model
  factory AgridDisplaySurvey.fromDict(Map<String, dynamic> dict) {
    final questions = (dict['questions'] as List).map((q) {
      ...
      final questionDescriptionContentType =
          AgridDisplaySurveyTextContentType.fromInt(questionContentTypeRaw);

      final buttonText = q['buttonText'] as String?;

      switch (type) {
        case 'link':
          return AgridDisplayLinkQuestion(
            id: id,
            question: question,
        ...
        case 'rating':
          return AgridDisplayRatingQuestion(
            id: id,
            question: question,
            ratingType:
                AgridDisplaySurveyRatingType.fromInt(q['ratingType'] as int),
        ...
        case 'single_choice':
          return AgridDisplayChoiceQuestion(
            id: id,
            question: question,
        ...
        case 'open':
        default:
          return AgridDisplayOpenQuestion(
            id: id,
            question: question,
            description: questionDescription,
            descriptionContentType: questionDescriptionContentType,
    ...
    AgridDisplaySurveyAppearance? appearance;
    if (dict['appearance'] != null) {
      ...
      final thankYouMessageDescriptionContentType =
          AgridDisplaySurveyTextContentType.fromInt(thankYouContentTypeRaw);

      appearance = AgridDisplaySurveyAppearance(
  ...
  final String id;
  final String name;
  final List<AgridDisplaySurveyQuestion> questions;
  final AgridDisplaySurveyAppearance? appearance;
