...
import 'package:agrid_flutter/src/surveys/survey_service.dart';
import 'package:agrid_flutter/src/util/logging.dart';
import 'surveys/models/agrid_display_survey.dart' as models;
...
/// An implementation of [AgridFlutterPlatformInterface] that uses method channels.
class AgridFlutterIO extends AgridFlutterPlatformInterface {
  AgridFlutterIO() {
    _methodChannel.setMethodCallHandler(_handleMethodCall);
  }

  /// The method channel used to interact with the native platform.
  final _methodChannel = const MethodChannel('agrid_flutter');
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    switch (call.method) {
      ...
      default:
        printIfDebug(
            '[Agrid] ${call.method} not implemented in AgridFlutterPlatformInterface');
        return null;
    }
  }
  ...
  Future<void> showSurvey(Map<String, dynamic> survey) async {
    ...
    final widget = AgridFlutterPlatformInterface.instance;
    if (widget is! AgridFlutterIO) {
      printIfDebug(
          'Cannot show survey: AgridFlutterPlatformInterface instance is not AgridFlutterIO');
      return;
    }

    final displaySurvey = models.AgridDisplaySurvey.fromDict(survey);

    // Try to show using SurveyService
    // This will work if the user has set up the AgridObserver correctly in their app
    await SurveyService().showSurvey(
      ...
      (survey, index, response) async {
        ...
        final nextQuestion = AgridSurveyNextQuestion(
  ...
  ///
  @override
  Future<void> setup(AgridConfig config) async {
