...
import 'package:agrid_flutter/src/error_tracking/dart_exception_processor.dart';
import 'package:agrid_flutter/src/error_tracking/agrid_exception.dart';
...
#0      Object.noSuchMethod (package:agrid-flutter:1884:25)
...
''');
      ...
      final result = DartExceptionProcessor.processException(
        ...
        inAppIncludes: ['agrid_flutter_example'],
        inAppExcludes: [],
        inAppByDefault: true,
      );
...
''');
    ...
    test('removes Agrid frames when stack trace is generated', () {
      final exception = Exception('Test exception');

      // Create a mock stack trace that includes Agrid frames
      final mockStackTrace = StackTrace.fromString('''
#0      DartExceptionProcessor.processException (package:agrid_flutter/src/error_tracking/dart_exception_processor.dart:28:7)
#1      AgridFlutterIO.captureException (package:agrid_flutter/src/agrid_flutter_io.dart:435:29)
#2      Agrid.captureException (package:agrid_flutter/src/agrid.dart:136:7)
#3      userFunction (package:my_app/main.dart:100:5)
#4      AgridFlutterIO.setup (package:agrid_flutter/src/agrid.dart:136:7)
...
''');
      ...
      expect(frames[1]['package'], 'agrid_flutter');
      expect(frames[1]['filename'], 'agrid.dart');
    ...
    test('processes AgridException with different mechanism types', () {
      final testCases = [
      ...
      for (final testCase in testCases) {
        ...
        final agridException = AgridException(
    ...
    test(
        'uses original error for stack trace processing when wrapped in AgridException',
        () {
    ...
    test('processes original error type correctly when wrapped', () {
      ...
      for (final originalError in testErrorTypes) {
        ...
        // Should extract type from original error, not AgridException
