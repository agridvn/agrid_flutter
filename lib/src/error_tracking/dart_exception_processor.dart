...
class DartExceptionProcessor {
  /// Converts Dart error/exception and stack trace to Agrid exception format
  static Map<String, dynamic> processException({
    ...
    // Extract Agrid metadata if error is wrapped in AgridException
    var mechanismType = 'generic';
    var handled = true;
    var currentError = error;

    if (error is AgridException) {
    ...
    if (effectiveStackTrace == null ||
      ...
      isGeneratedStackTrace = true; // Flag to remove top Agrid frames
    }
    ...
    final frames = hasValidStackTrace
        ? _parseStackTrace(
            ...
            removeTopAgridFrames: isGeneratedStackTrace,
          )
        : <Map<String, dynamic>>[];
    ...
    // - Stack trace was generated by Agrid (not from original exception)
  ...
  /// Determines if a stack frame belongs to Agrid SDK (just check package for now)
  static bool _isAgridFrame(Frame frame) {
    return frame.package == 'agrid_flutter';
  }
  ...
  /// Parses stack trace into Agrid format
  ///
  ...
  static List<Map<String, dynamic>> _parseStackTrace(
    ...
    List<String>? inAppExcludes,
    bool inAppByDefault = true,
    bool removeTopAgridFrames = false,
  }) {
    final chain = Chain.forTrace(stackTrace);
    final frames = <Map<String, dynamic>>[];

    for (final (index, trace) in chain.traces.indexed) {
      bool skipNextAgridFrame = removeTopAgridFrames;

      for (final frame in trace.frames) {
        // Skip top Agrid frames?
        if (skipNextAgridFrame) {
          if (_isAgridFrame(frame)) {
            continue;
          }
          skipNextAgridFrame = false;
        }

        final processedFrame = _convertFrameToAgrid(
          frame,
          inAppIncludes: inAppIncludes,
          inAppExcludes: inAppExcludes,
          inAppByDefault: inAppByDefault,
  ...
  /// Converts a Frame from stack_trace package to Agrid format
  static Map<String, dynamic>? _convertFrameToAgrid(
    Frame frame, {
    List<String>? inAppIncludes,
    List<String>? inAppExcludes,
    bool inAppByDefault = true,
