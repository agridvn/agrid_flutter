...
///
/// This service uses the AgridObserver to access the current navigation context
/// for displaying surveys. Users must add AgridObserver to their navigatorObservers list.
class SurveyService {
  ...
  /// Shows a survey using the AgridObserver context
  Future<void> showSurvey(
    ...
    if (_isShowingSurvey) {
      printIfDebug('[Agrid] A survey is already being displayed');
      return;
    }

    // Use the AgridObserver's context to show the survey
    if (AgridObserver.currentContext != null) {
      printIfDebug('[Agrid] Using AgridObserver context for survey');
      return _showSurveyWithNavigator(
        survey,
        onShown,
        onResponse,
        onClosed,
        AgridObserver.currentContext!,
      );
    }

    // If we can't show the survey, log an error
    printIfDebug(
        '[Agrid] Cannot show survey: No valid context found. To fix this make sure that you have installed AgridObserver correctly in your app.');
  }
  ...
  Future<void> _showSurveyWithNavigator(
    ...
    try {
      ...
      printIfDebug('[Agrid] Error showing survey: $e');
      _isShowingSurvey = false;
      _currentSurveyContext = null;
    }
  }
  ...
  Widget _buildSurveyWidget(
    ...
    return SurveyBottomSheet(
      ...
      onClosed: onClosed,
      appearance: SurveyAppearance.fromAgrid(survey.appearance),
    );
  }
