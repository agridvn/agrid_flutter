enum AgridPersonProfiles { never, always, identifiedOnly }

enum AgridDataMode { wifi, cellular, any }

class AgridConfig {
  final String apiKey;
  var host = 'https://us.i.agrid.com';
  var flushAt = 20;
  ...
  var debug = false;
  var optOut = false;
  var personProfiles = AgridPersonProfiles.identifiedOnly;
  ...
  var sessionReplayConfig = AgridSessionReplayConfig();

  /// iOS only
  var dataMode = AgridDataMode.any;

  /// Enable Surveys
  ///
  /// **Notes:**
  /// - After calling `Agrid().close()`, surveys will not be rendered until the SDK is re-initialized and the next navigation event occurs.
  /// - You must install `AgridObserver` in your app for surveys to display
  ///   - See: https://agrid.com/docs/surveys/installation?tab=Flutter#step-two-install-agridobserver
  /// - For Flutter web, this setting will be ignored. Surveys on web use the JavaScript Web SDK instead.
  ///   - See: https://agrid.com/docs/surveys/installation?tab=Web
  ///
  /// Defaults to true.
  var surveys = true;

  /// Configuration for error tracking and exception capture
  final errorTrackingConfig = AgridErrorTrackingConfig();

  // TODO: missing getAnonymousId, propertiesSanitizer, captureDeepLinks
  // onFeatureFlags, integrations

  AgridConfig(this.apiKey);
...
class AgridSessionReplayConfig {
...
class AgridErrorTrackingConfig {
