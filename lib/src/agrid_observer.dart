...
typedef AgridRouteFilter = bool Function(Route<dynamic>? route);

String? defaultNameExtractor(RouteSettings settings) => settings.name;

bool defaultAgridRouteFilter(Route<dynamic>? route) => route is PageRoute;

class AgridObserver extends RouteObserver<ModalRoute<dynamic>> {
  AgridObserver(
  ...
  /// Clears the current navigation context. Called when Agrid().close() is called.
  ///
  /// For internal use only. Should not be used by app developers.
  ///
  /// Note: Current limitation - After calling this method, Agrid will not have a valid BuildContext until
  ...
  final AgridRouteFilter _routeFilter;
  ...
  void _sendScreenView(Route<dynamic>? route) {
    ...
    if (_isTrackeableRoute(screenName)) {
      ...
      Agrid().screen(screenName: screenName);
    }
  }
